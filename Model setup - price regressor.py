...
#Convert 'Datum' to datetime and include the 
data_model['Datum'] = pd.to_datetime(data_model['Datum'])

#We use the data from january 1
data_model = data_model[data_model['Datum'] >= '2022-10-01']

#for the prophet model we need to rename the columns to 'ds' (date column) and to 'y' that is the target column
data_model['BetaaldePrijs'] = data_model['Verbruik KwH'] * data_model['Prijs']
data_model.rename(columns={'BetaaldePrijs': 'y', 'Datum': 'ds'}, inplace=True)
...
model = Prophet()
#model.add_regressor('Verbruik KwH')
...
initial = '180 days'  # e.g., 6 (180 days) months of data as the initial training period - Set histrorical data to train the model - caputing season but als have some left so we need  6 - 9 months of training data
